    ...
    require("typescript").setup({
        server = {
            on_attach = function(client, bufnr)
                u.map("n", "gs", ":TypescriptRemoveUnused<CR>", { buffer = bufnr })
                u.map("n", "gS", ":TypescriptOrganizeImports<CR>", { buffer = bufnr })
                u.map("n", "go", ":TypescriptAddMissingImports<CR>", { buffer = bufnr })
                u.map("n", "gA", ":TypescriptFixAll<CR>", { buffer = bufnr })
                u.map("n", "gI", ":TypescriptRenameFile<CR>", { buffer = bufnr })
                u.map("i", "${", change_template_string_quotes, { nowait = true, buffer = bufnr })

                api.nvim_buf_create_user_command(bufnr, "CssToJs", css_to_js, { range = true })
                u.map("n", "gx", ":set opfunc=v:lua.css_to_js<CR>g@", { buffer = bufnr })
                u.map("n", "gxx", ":CssToJs<CR>", { buffer = bufnr })
                u.map("v", "gx", ":CssToJs<CR>", { buffer = bufnr })
