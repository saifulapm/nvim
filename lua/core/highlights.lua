...
function M.adopt_winhighlight(win_id, target, name, fallback)
  ...
  local found = G.find(parts, function(part)
...
function M.plugin(name, hls)
  ...
  M.all(hls)
  G.augroup(fmt('%sHighlightOverrides', name), {
    {
      event = { 'ColorScheme' },
      command = function()
        M.all(hls)
...
local function set_sidebar_highlight()
  ...
  local st_color = M.alter_color(normal_bg, -16)
...
-- Color Scheme {{{1
-----------------------------------------------------------------------------//
if G.plugin_installed 'doom-one.nvim' then
